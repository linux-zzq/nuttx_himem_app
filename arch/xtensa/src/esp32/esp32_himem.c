...
#ifdef CONFIG_ESP32_HIMEM_MAPPING
  paddr_start -= esp_himem_mapping_reserved_area_size();
#endif
...
int esp_himem_realloc(size_t size, esp_himem_handle_t handle_in,
                      esp_himem_handle_t *handle_out)
{
  esp_himem_ramdata_t *r = handle_in;
  int blocks;
  int ok;
  uint16_t *block = NULL;
  ...
  blocks = size / CACHE_BLOCKSIZE;
  if (blocks <= r->block_ct)
    {
      return -EINVAL;
    }

  block = kmm_malloc(sizeof(uint16_t) * blocks);
  if (!block)
    {
      goto nomem;
    }

  memcpy(block, r->block, r->block_ct * sizeof(uint16_t));

  spinlock_flags = spin_lock_irqsave(NULL);
  ok = allocate_blocks(blocks -  r->block_ct, block + r->block_ct);
  spin_unlock_irqrestore(NULL, spinlock_flags);
  if (!ok)
      goto nomem;
