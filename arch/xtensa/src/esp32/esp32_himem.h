...
#  define SPIRAM_HIMEM_MAPPING_RESERVE 1
#else
#  define SPIRAM_HIMEM_MAPPING_RESERVE 0
#endif

#define PHYS_HIMEM_BLOCKSTART (128 - \
                               SPIRAM_BANKSWITCH_RESERVE - \
                               SPIRAM_HIMEM_MAPPING_RESERVE)
/****************************************************************************
...
/* Reallocate a block in high memory
 *
 * params:
 *   size Size of the to-be-allocated block, in bytes. Note that this
 *        needs to be a multiple of the external RAM mmu block size
 *        (32K).
 *   [in] Handle to the block of memory, as given by esp_himem_alloc
 *   [out] handle_out Handle to be returned
 *
 * returns:
 *   - ESP_OK if successful
